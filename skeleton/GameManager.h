...
private:
	...
	public:
		...
		void generateExplosion(RigidSolid* solid, list<RigidSolid*>& pieces) {
			ExplosionForceGenerator<RigidSolid>* explosion = new ExplosionForceGenerator<RigidSolid>(solid->getPos(), solid->getVel().magnitude()/2, 50, 0.8f);
			solidSystem->addForceGenerator(explosion);
			solidSystem->addParticlesToRegistry(pieces);
			ExplosionForceGenerator<Particle>* explosion_particle = new ExplosionForceGenerator<Particle>(solid->getPos(), solid->getVel().magnitude()/2, 30, 0.8f);
